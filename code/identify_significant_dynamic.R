...
for(igene in genes){
  ...
  estimate <- c(estimate, interaction_by_gene[[igene]][isnp, ]$Estimate)
  smallest_p_value_interaction <- c(smallest_p_value_interaction,  interaction_by_gene[[igene]][isnp, ]$adj_p_value)
...
final_df <- data.frame(genes, snps_interaction, estimate, smallest_p_value_interaction, genotype_pvalue, pseudotime_pvalue)
colnames(final_df) <- c("genes", "snps_interaction", "estimate", "p_value_interaction", "p_value_genotype", "p_value_pseudotime")
...
saveRDS(final_df, paste0("../results/eQTL_calling/", cell_type, "/all_hits.rds"))

sig_hits <- final_df[final_df$adj_p_value_interaction <= 0.1, ]

saveRDS(sig_hits, paste0("../results/eQTL_calling/", cell_type, "/significant_hits.rds"))
