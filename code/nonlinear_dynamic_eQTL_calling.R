...
for(i in c(start_pos:end_pos)){
  ...
  if(dim(geno_subset)[2] > 0){
    for(igeno in c(1:dim(geno_subset)[2])){
      ...
      eQTL.model.with.interaction <- lmer(expression ~ (. - individuals) + (1 | individuals), data = data_df)
      eQTL.model.no.interaction <- lmer(expression ~  (. - individuals - Gxt - `Gxt^2`) + (1 | individuals), data = data_df)
      teststat <- 2 * (as.numeric(summary(eQTL.model.with.interaction)$logLik) - 
                          as.numeric(summary(eQTL.model.no.interaction)$logLik))
      p.val <- pchisq(teststat, df = 2, lower.tail = FALSE)
      if(igeno == 1){
        p.values <- p.val
        snp <- colnames(geno_subset)[igeno]
      }else{
        p.values <- c(p.values, p.val)
        snp <- c(snp, colnames(geno_subset)[igeno])
      }}
    pvalues_by_gene[[index]] <- data.frame(snp, p.values)
   } else{
     pvalues_by_gene[[index]]<- NA }
  index <- index + 1
}

names(pvalues_by_gene) <- names(snps.select)[start_pos:end_pos]
saveRDS(pvalues_by_gene, paste0("/work-zfs/abattle4/prashanthi/sc-endo/results/eQTL_calling/", cell_type, "_quadratic/stegle/pvalues_by_gene_", start_pos, "to", end_pos, ".rds"))
